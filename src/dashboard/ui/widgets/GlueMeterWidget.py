...
class GlueMeterWidget(QWidget):
    """
    Pure UI widget: visual glue-level progress bar with a state indicator dot.

    Has zero knowledge of MessageBroker, topics, or any external system.
    Call set_weight() and set_state() to update the display.
    """
    ...
    # ------------------------------------------------------------------ #
    #  Public setter API (called by GlueMeterCard / DashboardWidget)      #
    # ------------------------------------------------------------------ #

    def set_weight(self, grams: float) -> None:
        """Update the displayed glue weight and fill percentage."""
        try:
    ...
    def set_state(self, state: str) -> None:
        """
        Update the circular status indicator colour.

        state: plain lowercase string e.g. "ready", "error", "disconnected".
        """
        try:
